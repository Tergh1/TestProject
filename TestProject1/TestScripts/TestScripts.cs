	...
	public partial class TestScripts
	{
		...
		public void TestCase1(string email)
		{
			...
			try
			{
				...
				{
					HomePage homePage = new HomePage(scope.driver);
					homePage.NavigateToHomePage();
					test.Info("Step1 completed: Visit http://www.musala.com/ ");

					scope.driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(60);
					homePage.ScrollAndClickContactUsButton();
		...
		public void TestCase2()
		{
			...
			try
			{
				using (var scope = new TestScope())
				{
					...
					homePage.NavigateToHomePage();
					test.Info("Step1 completed: Visit http://www.musala.com/ ");
					scope.driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(30);

					CompanyPage compPage = homePage.ClickCompanyTab();
					test.Info("Step2 completed: Click ‘Company’ tap from the top");
					Thread.Sleep(2000);

					Assert.AreEqual(Resources.Resources.COMPANYURL, scope.driver.Url);
					...
					string previousWindowHandle = scope.driver.CurrentWindowHandle;

					FacebookPage fPage = compPage.ClickFacebookLink();
					test.Info("Step5 completed: Click the Facebook link from the footer");

					foreach (string handle in scope.driver.WindowHandles)
					{
						if (handle != previousWindowHandle)
						{
							scope.driver.SwitchTo().Window(handle);
						}
					}
					...
					Thread.Sleep(1000);
					Assert.AreEqual(Resources.Resources.FACEBOOKURL, scope.driver.Url);
		...
		public void TestCase3(string name, string email, string mobile, string message)
		{
			...
			try
			{
				using (var scope = new TestScope())
				{
					...
					Assert.AreEqual(Resources.Resources.JOINUSURL, scope.driver.Url);
